...
func (r *deliveryRoutes) get(c *gin.Context) {
	...
	delivery, err := r.uc.GetDelivery(c.Request.Context(), id)
	if err != nil {
...
// @Description  Обновляет статус доставки (pending, in_progress, completed, failed)
// @Tags         deliveries
// @Accept       json
// @Produce      json
// @Param        id path int true "ID доставки"
// @Param        request body request.UpdateDeliveryStatus true "Новый статус"
...
// @Summary      Подтвердить загрузку товара
// @Description  Подтверждает загрузку товара из ячейки в дрон
// @Tags         deliveries
// @Accept       json
// @Produce      json
// @Param        request body request.ConfirmGoodsLoadedRequest true "ID заказа и ID ячейки"
