...
func Run(cfg *config.Config) {
	...
	orderUC := usecase.NewOrderUseCase(orderRepo, goodRepo, droneRepo, deliveryRepo, parcelAutomatRepo, lockerRepo, rabbitmqClient)
	droneUC := usecase.NewDroneUseCase(droneRepo)
	deliveryUC := usecase.NewDeliveryUseCase(deliveryRepo, orderRepo, lockerRepo, rabbitmqClient, notificationUC)
	lockerUC := usecase.NewLockerUseCase(lockerRepo)
	parcelAutomatUC := usecase.NewParcelAutomatUseCase(parcelAutomatRepo, lockerRepo, orderRepo, deliveryRepo, qrUC, orangePIAdapter)
