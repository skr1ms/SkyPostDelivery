...
func TestOrderUseCase_processPendingOrders_Success(t *testing.T) {
	...
	mockDroneRepo.On("UpdateStatus", ctx, droneID, "busy").Return(nil)
	mockDeliveryRepo.On("UpdateDrone", ctx, deliveryID, droneID).Return(nil)
	mockDeliveryRepo.On("UpdateStatus", ctx, deliveryID, "pending").Return(delivery, nil)
