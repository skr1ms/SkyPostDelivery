...
func TestDeliveryUseCase_StartDelivery_Success(t *testing.T) {
	...
	// Setup mocks for async executeDelivery goroutine
...
func TestDeliveryUseCase_ExecuteDelivery_Success(t *testing.T) {
	...
	mockNotifier.On("SendToDrone", ctx, droneID, mock.MatchedBy(func(msg map[string]interface{}) bool {
...
func TestDeliveryUseCase_ExecuteDelivery_NotifierError(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "droneNotifier.SendToDrone")
	mockNotifier.AssertExpectations(t)
}
