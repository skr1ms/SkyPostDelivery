...
home_pub = rospy.Publisher(
    '/drone/delivery/home_arrived', PoseStamped, queue_size=10)
...
if current_pose:
    ...
    rospy.loginfo("Home arrival notification published on /drone/delivery/home_arrived")
else:
    rospy.logwarn("No pose data available, skipping home arrival notification")

rospy.loginfo("="*60)
rospy.loginfo("RETURN TO BASE COMPLETED")
rospy.loginfo(f"Battery remaining: {battery_percentage:.1f}%")
rospy.loginfo("Drone status: IDLE and ready for next mission")
rospy.loginfo("="*60)
