...
func (c *Client) ValidateQR(ctx context.Context, qrData, parcelAutomatID string) (*entity.QRValidationResponse, error) {
	...
	if err := c.doRequest(ctx, "POST", url, req, &response); err != nil {
		return nil, fmt.Errorf("orchestrator client - ValidateQR: %w", err)
	}
...
func (c *Client) ConfirmPickup(ctx context.Context, cellIDs []uuid.UUID) error {
	...
	if err := c.doRequest(ctx, "POST", url, req, nil); err != nil {
		return fmt.Errorf("orchestrator client - ConfirmPickup: %w", err)
	}
...
func (c *Client) ConfirmLoaded(ctx context.Context, orderID, lockerCellID uuid.UUID) error {
	...
	if err := c.doRequest(ctx, "POST", url, req, nil); err != nil {
		return fmt.Errorf("orchestrator client - ConfirmLoaded: %w", err)
	}

	return nil
}

func (c *Client) doRequest(ctx context.Context, method, url string, reqBody, respBody interface{}) error {
	var lastErr error

	for attempt := 0; attempt <= c.retryCount; attempt++ {
		if attempt > 0 {
			c.logger.Warn("Retrying request to orchestrator", nil, map[string]interface{}{
				"attempt": attempt,
				"url":     url,
			})
		...
		if reqBody != nil {
			...
			if err != nil {
				return fmt.Errorf("orchestrator client - doRequest - marshal: %w", err)
			}
		...
		if err != nil {
			lastErr = fmt.Errorf("orchestrator client - doRequest - create request: %w", err)
			continue
		}
		...
		if err != nil {
			lastErr = fmt.Errorf("orchestrator client - doRequest - send request: %w", err)
			continue
		}
		...
		if err != nil {
			lastErr = fmt.Errorf("orchestrator client - doRequest - read response: %w", err)
			continue
		}

		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			lastErr = fmt.Errorf("orchestrator client - doRequest - bad status: %d, body: %s", resp.StatusCode, string(body))
			continue
		}

		if respBody != nil {
			if err := json.Unmarshal(body, respBody); err != nil {
				lastErr = fmt.Errorf("orchestrator client - doRequest - unmarshal response: %w", err)
				continue
			}
		}

		c.logger.Debug("Orchestrator request successful", nil, map[string]interface{}{
			"method": method,
			"url":    url,
		})
