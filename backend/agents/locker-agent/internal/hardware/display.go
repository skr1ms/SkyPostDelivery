...
func NewDisplayController(portName string, baudrate int, mockMode bool, log logger.Interface) (*DisplayController, error) {
	...
	if err != nil {
		log.Warn("Failed to open display serial port, switching to MOCK mode", err, map[string]interface{}{
			"port": portName,
		})
		return &DisplayController{
			mockMode: true,
	...
	log.Info("Display controller initialized", nil, map[string]interface{}{
		"port":     portName,
		"baudrate": baudrate,
	})
...
func (d *DisplayController) SendMessage(message string) error {
	if d.mockMode {
		d.logger.Info("MOCK Display message", nil, map[string]interface{}{
			"message": message,
		})
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("display controller - SendMessage: %w", err)
	}

	d.logger.Debug("Display message sent", nil, map[string]interface{}{
		"message": message,
	})
...
func (d *DisplayController) Close() error {
	...
	err := d.port.Close()
	if err != nil {
		return fmt.Errorf("display controller - Close: %w", err)
	}
