...
func NewArduinoController(portName string, baudrate int, mockMode bool, log logger.Interface) (*ArduinoController, error) {
	...
	if err != nil {
		log.Warn("Failed to open Arduino serial port, switching to MOCK mode", err, map[string]interface{}{
			"port": portName,
		})
		return &ArduinoController{
			mockMode: true,
	...
	log.Info("Arduino controller initialized", nil, map[string]interface{}{
		"port":     portName,
		"baudrate": baudrate,
	})
...
func (a *ArduinoController) sendCommand(command string) (string, error) {
	if a.mockMode {
		a.logger.Debug("MOCK Arduino command", nil, map[string]interface{}{
			"command": command,
		})
		return "OK", nil
	}
	...
	if err != nil {
		return "", fmt.Errorf("arduino controller - sendCommand - write: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("arduino controller - sendCommand - read: %w", err)
	}

	response := string(buf[:n])
	a.logger.Debug("Arduino response", nil, map[string]interface{}{
		"command":  command,
		"response": response,
	})
...
func (a *ArduinoController) OpenCell(cellNumber int) error {
	...
	if err != nil {
		return fmt.Errorf("arduino controller - OpenCell: %w", err)
	}

	a.logger.Info("Cell opened", nil, map[string]interface{}{
...
func (a *ArduinoController) OpenInternalDoor(doorNumber int) error {
	...
	response, err := a.sendCommand(command)
	if err != nil {
		return fmt.Errorf("arduino controller - OpenInternalDoor: %w", err)
	}

	a.logger.Info("Internal door opened", nil, map[string]interface{}{
...
func (a *ArduinoController) GetCellsCount() (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("arduino controller - GetCellsCount: %w", err)
	}

	var count int
	if _, err := fmt.Sscanf(response, "%d", &count); err != nil {
		return 0, fmt.Errorf("arduino controller - GetCellsCount - parse response: %w", err)
	}
...
func (a *ArduinoController) Reset() error {
	...
	if err != nil {
		return fmt.Errorf("arduino controller - Reset: %w", err)
	}
...
func (a *ArduinoController) Close() error {
	...
	err := a.port.Close()
	if err != nil {
		return fmt.Errorf("arduino controller - Close: %w", err)
	}
