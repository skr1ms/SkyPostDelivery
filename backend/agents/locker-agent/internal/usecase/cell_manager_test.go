...
func TestCellManagerUseCase_OpenCell(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMocks: func(repo *mocks.MockCellMappingInterface, arduino *mocks.MockArduinoInterface, display *mocks.MockDisplayInterface) {
				repo.On("IsInitialized").Return(true)
				repo.On("GetCellUUID", 999).Return(uuid.Nil, inmemory.ErrCellNotFound)
			},
			expectedErr: true,
		},
		{
...
func TestCellManagerUseCase_OpenInternalDoor(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMocks: func(repo *mocks.MockCellMappingInterface, arduino *mocks.MockArduinoInterface) {
				repo.On("IsInitialized").Return(true)
				repo.On("GetInternalCellUUID", 999).Return(uuid.Nil, inmemory.ErrCellNotFound)
			},
			expectedErr: true,
		},
	}
...
func TestCellManagerUseCase_PrepareCell(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMocks: func(repo *mocks.MockCellMappingInterface, arduino *mocks.MockArduinoInterface, display *mocks.MockDisplayInterface) {
				repo.On("IsInitialized").Return(true)
				repo.On("GetCellNumber", cellUUID).Return(0, entity.CellType(""), inmemory.ErrCellNotFound)
			},
			expectedErr: true,
		},
	}
...
func (m *mockLogger) Info(message string, err error, fields ...map[string]interface{})  {}
func (m *mockLogger) Warn(message string, err error, fields ...map[string]interface{})  {}
func (m *mockLogger) Error(message string, err error, fields ...map[string]interface{}) {}
func (m *mockLogger) Debug(message string, err error, fields ...map[string]interface{}) {}
