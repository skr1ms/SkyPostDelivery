...
func (uc *QRScannerUseCase) ProcessQRScan(ctx context.Context, qrData string) (*entity.QRScanResponse, error) {
	if !uc.cellManager.IsInitialized() {
		return &entity.QRScanResponse{
			Success: false,
			Message: "Service not initialized. Cell mapping not synced.",
		}, nil
	}
	...
	uc.logger.Debug("Validating QR with orchestrator", nil, map[string]interface{}{
	...
	if err != nil {
		...
		return nil, fmt.Errorf("qr scanner usecase - ProcessQRScan - orchestrator.ValidateQR: %w", err)
	}

	if !validationResp.Success {
		uc.logger.Warn("QR validation failed", nil, map[string]interface{}{
...
func (uc *QRScannerUseCase) ConfirmPickup(ctx context.Context, cellIDs []string) error {
	...
	for _, cellIDStr := range cellIDs {
		cellUUID, err := uuid.Parse(cellIDStr)
		if err != nil {
			return fmt.Errorf("qr scanner usecase - ConfirmPickup - parse cell ID: %w", err)
		}
		cellUUIDs = append(cellUUIDs, cellUUID)
	}

	if err := uc.orchestratorClient.ConfirmPickup(ctx, cellUUIDs); err != nil {
		return fmt.Errorf("qr scanner usecase - ConfirmPickup - orchestrator: %w", err)
	}

	uc.logger.Info("Pickup confirmed successfully", nil, map[string]interface{}{
...
func (uc *QRScannerUseCase) ConfirmLoaded(ctx context.Context, orderID, lockerCellID string) error {
	...
	if err != nil {
		return fmt.Errorf("qr scanner usecase - ConfirmLoaded - parse order ID: %w", err)
	}

	cellUUID, err := uuid.Parse(lockerCellID)
	if err != nil {
		return fmt.Errorf("qr scanner usecase - ConfirmLoaded - parse locker cell ID: %w", err)
	}

	if err := uc.orchestratorClient.ConfirmLoaded(ctx, orderUUID, cellUUID); err != nil {
		return fmt.Errorf("qr scanner usecase - ConfirmLoaded - orchestrator: %w", err)
	}

	uc.logger.Info("Load confirmed successfully", nil, map[string]interface{}{
		"order_id":       orderID,
...
func (uc *QRScannerUseCase) handleQRResults(ctx context.Context) {
	...
	for {
		...
		case qrData := <-resultChan:
			...
			if !uc.isValidJSON(qrData) {
				uc.logger.Warn("Invalid QR data format", nil, map[string]interface{}{
					"data_length": len(qrData),
				})
				continue
			}

			uc.logger.Info("QR code detected, processing", nil, map[string]interface{}{
				"data_length": len(qrData),
			})
...
func (uc *QRScannerUseCase) isValidJSON(data string) bool {
